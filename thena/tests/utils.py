...
def google_auth(url, request):
    ...
    if access_token:
        content = {'data': {'email': 'fake@email.com', 'isVerified': True}}
    else:
        content = {'error': {'code': 401, 'message': 'Login Required', 'errors': [{'locationType': 'header', 'domain': 'global', 'message': 'Login Required', 'reason': 'required', 'location': 'Authorization'}]}}
